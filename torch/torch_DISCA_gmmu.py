...
def prepare_training_data(x_train, labels_temp_proba, labels, n):
    ...
    label_one_hot = one_hot(labels, len(labels)) 
    ...
    # # labels_tile=[5,10,3,9,6,11,0,4,7,1,8,2,4,8,2]
    
    
    # _neg = data_augmentation(x_train, n + 1)[x_train.shape[0]:]# _neg.shape=(15,24,24,24,1)
    # x_train_augmented_neg = _neg[index_negative]        # 只是为了交换顺序

    x_train_augmented_neg = x_train_augmented_pos

    np.random.shuffle(index)              
    
    x_train_permute = [x_train_augmented[index].copy(), x_train_augmented_pos[index].copy(), x_train_augmented_neg[index].copy()] 

    # x_train_permute = [x_train_augmented[index].detach().numpy().copy(), x_train_augmented_pos[index].copy(), x_train_augmented_neg[index].copy()] 
...
class YOLO_Model(nn.Module):
    def __init__(self, hidden_channel=32):
        ...
        # ###############################################################
        # ###############################################################
        # self.conv11 = nn.Conv3d(1, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn11 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv12 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn12 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv13 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn13 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))


        # self.conv14 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn14 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv15 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn15 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv16 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn16 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv17 = nn.Conv3d(self.hidden_channel+12, self.hidden_channel+12, (3, 3, 3), stride=1, padding=1)
        # self.bn17 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel+12),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))
        # ###############################################################
        ...
        # self.conv18 = nn.Conv3d(1, self.hidden_channel*2, (3, 3, 3), stride=1, padding=1)
        # self.bn18 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*2),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))


        # self.conv19 = nn.Conv3d(self.hidden_channel*2, self.hidden_channel*2, (3, 3, 3), stride=1, padding=1)
        # self.bn19 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*2),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv20 = nn.Conv3d(self.hidden_channel*2, self.hidden_channel*2, (3, 3, 3),  stride=1, padding=1)
        # self.bn20 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*2),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv21 = nn.Conv3d(self.hidden_channel*2, self.hidden_channel*2, (3, 3, 3),  stride=1, padding=1)
        # self.bn21 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*2),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv22 = nn.Conv3d(self.hidden_channel*2, self.hidden_channel*2, (3, 3, 3),  stride=1, padding=1)
        ...
        # self.conv23 = nn.Conv3d(self.hidden_channel*2, self.hidden_channel*3, (3, 3, 3),  stride=1, padding=1)
        # self.bn23 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*3),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv24 = nn.Conv3d(self.hidden_channel*3, self.hidden_channel*3, (3, 3, 3),  stride=1, padding=1)
        # self.bn24 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*3),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))

        # self.conv25 = nn.Conv3d(self.hidden_channel*3, self.hidden_channel*3, (3, 3, 3),  stride=1, padding=1)
        # self.bn25 = nn.Sequential( 
        #     nn.ReLU(),
        #     nn.BatchNorm3d(self.hidden_channel*3),
        #     nn.MaxPool3d(kernel_size=self.image_size, stride=1))
    ...
    def forward(self,input):    # input=[3,15,1,24,24,24]
        ...
        x = input
        x = self.conv1(x)
        m1 = self.bn1(x)    # m1.shape=[15,32,3,3,3]

        x = self.conv2(x)
        m2 = self.bn2(x)    # m2.shape=[15,32,5,5,5]

        x = self.conv3(x)
        m3 = self.bn3(x)    # m3.shape=[15,32,7,7,7]

        x = self.conv4(x)
        m4 = self.bn4(x)    # m1.shape=[15,32,9,9,9]


        x = self.conv5(x)
        m5 = self.bn5(x)    # m1.shape=[15,32,11,11,11]



        x = self.conv6(x)
        m6 = self.bn6(x)    # m1.shape=[15,32,13,13,13]

        x = self.conv7(x)
        m7 = self.bn7(x)    # m1.shape=[15,32,15,15,15]

        x = self.conv8(x)
        m8 = self.bn8(x)    # m1.shape=[15,32,17,17,17]

        x = self.conv9(x)
        m9 = self.bn9(x)    # m1.shape=[15,32,19,19,19]

        x = self.conv10(x)
        m10 = self.bn10(x)  # m1.shape=[15,32,21,21,21]

        # #############################
        # x = self.conv11(input)
        # m11 = self.bn11(x)  # m11.shape=[15,44,3,3,3]

        # x = self.conv12(x)
        # m12 = self.bn12(x)  # m11.shape=[15,44,5,5,5]

        # x = self.conv13(x)
        # m13 = self.bn13(x)  # m11.shape=[15,44,7,7,7]

        # x = self.conv14(x)
        # m14 = self.bn14(x)  # m11.shape=[15,44,9,9,9]

        # x = self.conv15(x)
        # m15 = self.bn15(x)  # m11.shape=[15,44,11,11,11]

        # x = self.conv16(x)
        # m16 = self.bn16(x)  # m11.shape=[15,44,13,13,13]

        # x = self.conv17(x)
        # m17 = self.bn17(x)  # m11.shape=[15,44,14,14,14]

        # x = self.conv18(input)
        # m18 = self.bn18(x)  # m11.shape=[15,64,5,5,5]

        # x = self.conv19(x)
        # m19 = self.bn19(x)  # m11.shape=[15,64,9,9,9]

        # x = self.conv20(x)
        # m20 = self.bn20(x)  # m11.shape=[15,64,13,13,13]

        # x = self.conv21(x)
        # m21 = self.bn21(x)  # m11.shape=[15,64,13,13,13]

        # x = self.conv22(x)
        # m22 = self.bn22(x)  # m11.shape=[15,64,17,17,17]

        # x = self.conv23(x)
        # m23 = self.bn23(x)  # m11.shape=[15,96,8,8,8]

        # x = self.conv24(x)
        # m24 = self.bn24(x)  # m11.shape=[15,96,13,13,13]

        # x = self.conv25(x)
        # m25 = self.bn25(x)  # m11.shape=[15,96,18,18,18]

        # x_concat = torch.cat((m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12,m13,m14,m15,m16,m17,m18,m19,m20,m21,m22,m23,m24,m25), 1)
