...
parser.add_argument('-c', '--config_yaml', type=str, default=r'/home/lab2/zyc/A_orange/DISCA_GMM/config/train_exam.yaml', help='YAML config file')
...
x_train = h5f['dataset_1'][16065:] 
infos = infos[16065:]       # info.shape = [16265]
...
for i in tqdm(range(np.max(labels) + 1)):
    ...
    for j in locs:              # j[2]包含一个三维坐标, s=16, 
        # print("j=",j)
        if j[0] == 'emd_4603.map': #emd_4603_deconv_corrected.mrc / emd_4603.map
            v_i[j[2][0] - s: j[2][0] + s, j[2][1] - s: j[2][1] + s, j[2][2] - s: j[2][2] + s] = v[j[2][0] - s: j[2][0] + s, j[2][1] - s: j[2][1] + s, j[2][2] - s: j[2][2] + s]
...
for model_name in model_names:
    ...
    with torch.no_grad():
        for a in range(total_num):
            ...
            x_train = x_train.to(device)    # [197,24,24,24,1]
            x_train = x_train.permute(0,4,1,2,3)  
            features = yopo(x_train)        # features=[40,32]
    ...
    # x_train = torch.tensor(x_train,dtype=torch.float32).to(device)    # [197,24,24,24,1]
    # x_train = x_train.permute(0,4,1,2,3)  
    # features = yopo(x_train)        # features=[200,32]
