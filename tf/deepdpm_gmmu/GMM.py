...
class GMM(tf.keras.models.Model):
    """
    ...
    def _init_params(self):
        if self.mu_init is not None:
            ...
            self.mu = self.add_weight(name='mu', shape=(1, self.n_components, self.n_features), dtype=DATA_TYPE,
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                assert self.var_init.shape == (1, self.n_components, self.n_features), \
                    "Input var_init does not have required tensor dimensions (1, %i, %i)" % (
                        self.n_components, self.n_features)
                self.var = self.add_weight(name='var', shape=self.var_init.shape, dtype=DATA_TYPE,
                                           initializer=tf.initializers.constant(self.var_init), trainable=False)
            else:
                self.var = self.add_weight(name='var', shape=(1, self.n_components, self.n_features), dtype=DATA_TYPE,
            ...
            if self.var_init is not None:
                ...
                var_init = tf.cast(tf.tile(tf.reshape(tf.eye(self.n_features, dtype=DATA_TYPE),
                                                (1, 1, self.n_features, self.n_features)),
                                     (1, self.n_components, 1, 1)), DATA_TYPE)
                self.var = self.add_weight(name='var', shape=var_init.shape, dtype=DATA_TYPE,
                                           initializer=tf.initializers.constant(var_init), trainable=False)

        # (1, k, 1)
        self.pi = self.add_weight(name='pi', shape=(1, self.n_components, 1), trainable=False, dtype=DATA_TYPE,
    ...
    def __check_inv(self):
        result = tf.linalg.pinv(self.var)
    ...
    def fit(self, x, warm_start=False):
        ...
        self.batch = min(self.batch, x.shape[0])
        if self.batch == 0:
    ...
    def model_param(self):
        """
        Return the pi, mean, variance after training
        returns:
            pi:               tf.Tensor (1, k)
            mu:               tf.Tensor (1, k, d)
            var:              tf.Tensor (1, k, d) or tf.Tensor (1, k, d, d)
        """
    ...
    def predict(self, x, probs=False):
        ...
        if probs:
            ...
            result = tf.squeeze(p_k / (tf.reduce_sum(p_k, axis=1, keepdims=True)))
            return tf.where(tf.math.is_nan(result), 0., result)
        else:
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            mu = self.mu
            var = self.var
            precision = tf.cast(tf.linalg.inv(var), DATA_TYPE)
            d = x.shape[-1]

            log_2pi = d * tf.math.log(2. * pi)
            log_2pi = tf.cast(log_2pi, DATA_TYPE)
            log_det = self._calculate_log_det(precision)
            log_det = tf.cast(log_det, DATA_TYPE)


            x = tf.cast(x, DATA_TYPE)
            mu = tf.cast(mu, DATA_TYPE)
    ...
    def _m_step(self, x, log_resp):
        ...
        resp = tf.cast(resp, DATA_TYPE)
        pi = tf.cast(pi, DATA_TYPE)
        x = tf.cast(x, DATA_TYPE)
    ...
    def get_kmeans_mu(self, x, n_centers, init_times=50, min_delta=1e-3):
        ...
        min_cost = np.inf
        #print("X shape:", x.shape, "n_center", n_centers)
